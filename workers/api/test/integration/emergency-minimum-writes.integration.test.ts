...
describe('emergency minimum write mode', () => {
  it('skips an identical next-day home-loan row while advancing current state by default', async () => {
  ...
  it('respects explicit minimum-write side-effect suppression for unchanged rows', async () => {
    await resetHomeWriteTables()

    const base = {
      ...homeFixture(),
      bankName: 'ANZ',
      productId: `home-minimum-${crypto.randomUUID()}`,
      sourceUrl: 'https://api.anz/cds-au/v1/banking/products/home-minimum',
      productUrl: 'https://www.anz.com.au/personal/home-loans/',
      runSource: 'scheduled' as const,
      retrievalType: 'present_scrape_same_date' as const,
      fetchEventId: 401,
    ...
    const initial = await upsertHistoricalRateRows(env.DB, [first])
    const repeated = await upsertHistoricalRateRows(env.DB, [second], {
    ...
    const historical = await env.DB
      ...
      .first<{ n: number }>()
    const latest = await env.DB
      .prepare('SELECT collection_date FROM latest_home_loan_series WHERE product_id = ?')
      .bind(base.productId)
      .first<{ collection_date: string }>()
    const interval = await env.DB
      .prepare('SELECT last_confirmed_collection_date FROM home_loan_rate_intervals WHERE product_id = ?')
      .bind(base.productId)
      .first<{ last_confirmed_collection_date: string }>()
    const productPresence = await env.DB
      .prepare('SELECT last_seen_collection_date FROM product_presence_status WHERE section = ? AND bank_name = ? AND product_id = ?')
      .bind('home_loans', base.bankName, base.productId)
      .first<{ last_seen_collection_date: string }>()
