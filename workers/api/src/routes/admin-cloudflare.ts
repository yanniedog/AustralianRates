...
adminCloudflareRoutes.get('/cloudflare/d1-usage', async (c) => {
  ...
  try {
    ...
    const filledUsageDays = fillUsageDateRange(usageDays, dateDaysAgo(Math.min(days, GRAPHQL_MAX_HISTORY_DAYS)), now)
    const sorted = addQuotaFields(sortDaysByDate(filledUsageDays), cycleStartDay)
