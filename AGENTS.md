...
- **Watch CI feedback** (`gh pr checks <num> --watch`) and respond to every failure and review comment on the same branch until green. **PR review bots** (automated reviewers on GitHub) count as peer review: resolve or fully address **every bot thread** before squash-merge to **`main`**—not only when **CI** is green. See `.cursor/rules/git-pr-workflow-default.mdc` (“PR review bots”).
...
**Default:** land work via a **feature branch** and a **pull request into `main`** (not by pushing commits straight to `main`). Sync `main`, `git checkout -b agent/<slug>` (or `feat/…` / `fix/…`), commit, `git push -u origin HEAD`, open PR (`gh pr create --base main` or GitHub UI), fix until **PR CI** is green **and automated PR review feedback is fully handled** before squash-merge. For branches named **`agent/*`**, **`feat/*`**, or **`fix/*`**, GitHub Actions (`.github/workflows/pr-auto-merge.yml`) enables **squash auto-merge** when the PR is not a draft; after **`ci_result`** passes and the repo has **Allow auto-merge** plus required **`ci_result`** on **`main`** (see `docs/CONCURRENT_AGENT_WORKFLOW.md`), GitHub may merge without a separate merge click. **Assistants:** do not enable or allow that merge to complete until **PR review bot threads are fully handled**—GitHub may not gate on bots; peer-review policy does (`.cursor/rules/git-pr-workflow-default.mdc`). Then **delete head branch** (if enabled on the repo) clears the remote branch. **`.github/workflows/stale-branch-cleanup.yml`** runs on **pushes to `main`**, weekly, and on manual dispatch; it deletes lingering remote heads under those prefixes when the branch had a **merged** PR and has **no open PR**. **After a merge**, run **`npm run git:graph-hygiene`** (prunes stale remote-tracking refs and removes local branches whose upstream is gone) so Git Graph stays readable. Full steps: `docs/CONCURRENT_AGENT_WORKFLOW.md` and `.cursor/rules/git-pr-workflow-default.mdc`.
