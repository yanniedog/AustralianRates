...
- **Watch CI feedback** (`gh pr checks <num> --watch`) and respond to every failure and review comment on the same branch until green. After each push, **wait for automated PR bots to finish** (typically a few minutes; you may do other work, then re-check the PR) before treating bot review as complete or giving a final summary to the user.
