...
### 4.1 Executive summary (repeated)

The admin export center provides three streams (canonical, optimized, operational) with snapshot and delta (where supported), artifact storage in R2, and operational single-bundle or client-side concat download. Critical gaps: no retry for failed jobs (P0), weak delta cursor discoverability and no persistence (P1), and artifact metadata not shown in the UI (P1). Exports are not sufficient for full reconstruction without schema (migrations), a D1 import script, optional R2 restore script, and clear documentation.

### 4.2 UX/product actions

| Do this | File/owner |
|---------|-------------|
| Add retry for failed jobs: "Retry" button and POST /admin/downloads/:jobId/retry. | site/admin/exports.html (UI), workers/api/src/routes/admin-downloads.ts (API) |
| Improve delta cursor: hint text and optional "Use latest" that pre-fills since_cursor from latest completed job in section. | site/admin/exports.html |
| Show artifact metadata (row count, size, cursor range) in job cards. | site/admin/exports.html |
| Clarify operational scope/mode: remove disabled scope control or add hint that operational is always full DB, snapshot only. | site/admin/exports.html |
| Optional: progress or clearer errors for client-side operational bundle concat; optional "Dismiss" for message; optional polling indicator. | site/admin/exports.html |

### 4.3 API/backend actions

| Do this | File/owner |
|---------|-------------|
| Add POST /admin/downloads/:jobId/retry for failed jobs (requeue + continue). | workers/api/src/routes/admin-downloads.ts |
| Document error codes, HTTP usage, and response contract (list, job, artifact download, bundle). | AGENTS.md or docs/admin-api.md / docs/admin-export-api.md |
| Optional: document or enforce max since_cursor; document GET/DELETE query params and limits. | workers/api/src/routes/admin-downloads.ts, docs |

### 4.4 Reconstruction & backup actions

| Do this | File/owner |
|---------|-------------|
| Add a doc (e.g. docs/admin-export-reconstruction.md) describing what each stream exports, that DDL is not included, and that full reconstruction requires migrations + D1 import script + optional R2 script. | docs/ |
| Differentiate admin JSONL exports from wrangler D1 export (disaster recovery vs application-level export). | docs/backup.md or docs/admin-export-reconstruction.md |
| Provide a D1 import script that applies operational (or canonical/optimized) JSONL to an existing D1 schema. | New script (e.g. under scripts/ or tools/), referenced from docs |
| If R2 restore is required, provide a script that reads canonical payload JSONL and writes bodies to R2. | New script, referenced from docs |

### 4.5 Summary table

| Area | Current state | Action (one line) | Owner/place |
|------|---------------|-------------------|-------------|
| Retry failed jobs | No retry; user must create new job | Add Retry button and POST /admin/downloads/:jobId/retry | exports.html, admin-downloads.ts |
| Delta cursor | Number input, copy from completed job only | Hint + optional "Use latest" and pre-fill from latest end_cursor | exports.html |
| Artifact metadata | In API only | Show row count, size, cursor range in job card UI | exports.html |
| Messaging | Single msg, 5s auto-hide, success/error | Optional Dismiss; ensure error text clear | exports.html |
| Polling | 5s when pending | Optional "Auto-refreshing" indicator | exports.html |
| Operational scope | Disabled select, "all" only | Hint or remove control; state snapshot-only | exports.html |
| Bundle fallback | Client concat when no download_path | Clear part-failure errors; optional progress | exports.html |
| API errors | Consistent jsonError shape, 4xx/5xx | Document codes and response contract | AGENTS.md or docs |
| API validation | stream/scope/mode validated; since_cursor unbounded | Document params; optional cap on since_cursor | admin-downloads.ts, docs |
| Retry API | None | POST :jobId/retry for failed jobs | admin-downloads.ts |
| Reconstruction | Exports are JSONL only; no DDL, no import script | Doc + D1 import script + optional R2 script | docs/, new scripts |
| Backup vs admin export | backup.md = wrangler D1 only | Document admin exports vs D1 export for recovery | docs/backup.md or new doc |
