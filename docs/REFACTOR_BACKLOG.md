...
| `workers/api/src/db/historical-rates.ts` | 707 | Touched by change-aware write work and over the 500 LOC refactor trigger. Split home-loan preparation, post-write side effects, and batch/fallback orchestration into sibling modules after the shared write semantics are stable. |
