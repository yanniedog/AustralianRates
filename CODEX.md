...
Match **Cursor** and **Claude** instructions: **feature branch** off `main`, **push the branch**, open a **PR to `main`**, keep **GitHub Actions** green on that PR, resolve **PR review bot** threads, then **merge** when ready. Details: **`docs/CONCURRENT_AGENT_WORKFLOW.md`** (**CI vs PR review bots**) and **`.cursor/rules/git-pr-workflow-default.mdc`** (“PR review bots”).
