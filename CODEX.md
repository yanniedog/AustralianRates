...
Match **Cursor** and **Claude** instructions: **feature branch** off `main`, **push the branch**, open a **PR to `main`**, keep **GitHub Actions** green on that PR, **enumerate and close each review thread** (implement or reply—see **`.cursor/rules/respond-to-each-review-comment.mdc`**), **merge** when ready. Details: **`docs/CONCURRENT_AGENT_WORKFLOW.md`** and **`.cursor/rules/git-pr-workflow-default.mdc`**.
