...
async function prepareWorkspace(page: Page): Promise<void> {
...
type LayoutPayload = {
  chartPanelActive: boolean
  context: string
  failures: string[]
  warnings: string[]
}

function evaluateLayout(context: string, workspace: boolean): LayoutPayload {
  ...
  for (const sel of landmarks) {
    ...
    if (r.left < -2 || r.right > vw + 2) {
      failures.push(`landmark clipped horizontally: ${sel} (left=${r.left.toFixed(0)} right=${r.right.toFixed(0)} vw=${vw})`)
    }
  ...
  document.querySelectorAll(textRoots).forEach((el) => {
    ...
    const hiddenOverflow = cs.overflow === 'hidden' || cs.overflow === 'clip' || cs.overflowX === 'hidden' || cs.overflowY === 'hidden'
    if (hiddenOverflow) {
      ...
      for (const vp of VIEWPORTS) {
        ...
        try {
          ...
          const payload = await page.evaluate(
            ({ ctx, workspace }) => {
              const g = (globalThis as unknown as { __arLayoutEval?: () => LayoutPayload })
              g.__arLayoutEval = () => evaluateLayout(ctx, workspace)
              return g.__arLayoutEval()
            },
            { ctx: context, workspace: route.workspace },
          )
