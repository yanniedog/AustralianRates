...
Git/GitHub:
- **Default (mandatory unless the user waives or orders a `main` hotfix):** feature branch off `main` → commit → push branch → **PR to `main`** → merge when **PR CI** is green **and automated PR review (bot) feedback is fully addressed**. See `docs/CONCURRENT_AGENT_WORKFLOW.md`, repo `AGENTS.md` (“Default git workflow”), and Cursor `.cursor/rules/git-pr-workflow-default.mdc` (“PR review bots”). Use `gh pr create`, `gh pr checks watch`, `gh pr merge` (e.g. `--auto --squash`) when available.
