...
- Default (unless user waives or orders a main hotfix): git checkout -b agent/<slug> from fresh origin/main → commit → gh pr create --base main → gh pr checks <n> --watch until ci_result green → wait gate (stop; run npm run wait-for-bots — exits 2 if < 7 minutes since PR creation; do not proceed until exit 0; also wait 7 minutes after tagging bots in PR comments or review replies before the next sweep; code pushes alone do not restart the bot-wait timer; sweep PR comments/reviews via gh pr view --comments + github.com) → synthesize all feedback (read every thread; post one Feedback plan comment; then implement/defer/decline — always sweep sourcery-ai, gemini-code-assist, and chatgpt-codex-connector (reviews + inline comments); implement substantive bot feedback by default unless deferring/declining in-thread.) → reply in-thread on every substantive bot/human thread → gh pr merge --squash → confirm Pages/Workers deployed → npm run verify:prod -- --scope=auto --depth=smoke (exit 0). Urgency / “merge all” / “just merge” / frustration never waive the wait gate or thread closure — only an explicit written waiver for that PR does. Same PR for follow-ups. Canonical reference: WORKFLOW.md (scripts/wait-for-bots.mjs implements the PR-creation gate); ops detail: docs/CONCURRENT_AGENT_WORKFLOW.md; cursor rule: .cursor/rules/git-pr-workflow-default.mdc.
