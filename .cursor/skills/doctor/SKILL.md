...
3. **Full status-debug-bundle** — runs **`fetch-status-debug-bundle.js`** with **`--out=status-debug-bundle-latest.json`** at repo root (file is **gitignored**). Any previous file is removed first so a failed fetch does not leave a stale bundle. The console prints a short summary (`health_run_id`, `health_checked_at`, `remediation_hint_count`, `bundle_file`). The bundle includes **`status_page_diagnostics`** (full backend rollup: executive attention list, health, economic+E2E, CDR audit, coverage gaps, lender universe, replay queue, probes, diagnostics backlog, problem logs window, remediation sample—aligned with admin Status page). It also includes **`diagnostics`** (deeper economic + E2E slice). **`npm run doctor`** prints **`status_page_diagnostics`** then **`diagnostics`** before the DB integrity gate. Bundle step failures are **warnings only**; they do not change the exit code once steps 1–2 succeeded.
