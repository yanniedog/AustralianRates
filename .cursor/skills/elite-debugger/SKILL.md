...
- Per .cursor/rules/fix-commit-verify-loop.mdc: after any production-affecting fix, run the loop until the problem is **definitely** fixed. Do not ask the user to reproduce; verify yourself.
  ...
  2. **Commit and push** — Default: **feature branch + PR to `main`** per `.cursor/rules/git-pr-workflow-default.mdc` (merge when PR CI green **and PR review bot threads replied to and resolved**). **Only** if the user explicitly ordered a **`main` hotfix:** `git checkout main`, `git add`, `git commit`, `git push origin main`.
