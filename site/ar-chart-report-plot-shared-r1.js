(function () {
    ...
    function render(options) {
        ...
        function showRibbonIdlePanel() {
            ...
            var spotlightSummary = ribbonAutoSpotlightBank ? ribbonHoverSummaryText(ribbonAutoSpotlightBank) : '';
            setRibbonHierarchyLayoutActive(true);
            ribbonHierarchyPanel.show({
                heading: 'Select a lender',
                meta: spotlightSummary || 'Use the ribbon chips or chart bands to open the tier tree.',
                compact: true,
                renderBody: function (wrap) {
                    var card = document.createElement('div');
                    card.className = 'ar-report-underchart-tree-empty';
                    card.style.cssText = [
                        'display:grid',
                        'gap:8px',
                        'padding:10px 12px',
                    ...
                    lead.textContent = 'Tier tree is hidden until a lender is pinned.';
                    lead.style.cssText = 'font-size:12px;line-height:1.35;';
                    card.appendChild(lead);

                    var note = document.createElement('span');
                    note.textContent = 'Click a lender chip, a ribbon band, or a spotlight card.';
                    note.style.cssText = 'font-size:11px;line-height:1.45;color:' + String(theme.muted || '#94a3b8') + ';';
