 ...
 *
 * View modes:
 *   'bank'     — Best TD rate per bank for preferred term (default)
...
(function () {
    ...
    function render(container, model, fields, rbaHistory, cpiData, economicOverlaySeries) {
        ...
        var bankMax = null;
        lines.forEach(function (b) { b.points.forEach(function (p) { if (!bankMax || p.date > bankMax) bankMax = p.date; }); });
        ...
        var chart = L.createChart(mount, {
            ...
            localization: { priceFormatter: function (p) { return Number(p).toFixed(2) + '%'; }, timeFormatter: function (time) { return fmtFull(M.utcToYmd(time)); } },
        ...
        if (cpiPts.length) {
            ...
            cpiSeriesApi.setData(M.fillForwardDaily(cpiPts, 'date', 'value', rbaStart, ctxMax).map(function (p) { return { time: M.ymdToUtc(p.date), value: p.value }; }));
        }
        ...
        if (rbaData.points.length) {
            ...
            rbaSeriesApi.setData(M.fillForwardDaily(rbaData.points, 'date', 'rate', rbaStart, ctxMax).map(function (p) { return { time: M.ymdToUtc(p.date), value: p.value }; }));
        }
        ...
        var state = {
            ...
            dispose: function () { if (disposed) return; disposed = true; ro.disconnect(); try { chart.remove(); } catch (_) {} container._reportDispose = null; },
        };
