...
`site-variant.js` is also the required bootstrap for Clarity analytics on public and admin HTML pages. `npm run build` and `npm run check:clarity` fail if that integration or its privacy disclosure is removed.
