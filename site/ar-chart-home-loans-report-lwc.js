...
(function () {
    ...
    function buildBankSeries(model) {
        ...
        allSeries.forEach(function (s) {
            ...
            (s.points || []).forEach(function (p) {
                ...
                byBank[k].byDate[d] = M.mergeWinningMortgage(byBank[k].byDate[d], v, pn, p.row || null);
            });
        });
        return Object.keys(byBank)
            ...
            .map(function (entry, index) {
                ...
                entry.section = 'home-loans';
                entry.selectionKey = String(entry.bankName || '').trim().toLowerCase();
                return entry;
            });
    }
    ...
    function render(container, model, fields, rbaHistory, cpiData, economicOverlaySeries) {
        ...
        visiLines.forEach(function (bank) {
            var clippedPts = clipSteppedPoints(bank.points, windowStart, ctxMax);
            ...
            seriesApis.push({
                ...
                baseColor: bank.color,
                baseLineWidth: lineWidth,
                selectionKey: bank.selectionKey,
            });
        });
        ...
        function buildMacroItems(rbaVal, cpiVal, ymd) {
            var items = [];
            if (rbaVal != null) { var p = M.prevStepValue(rbaData.points, ymd || ctxMax, 'rate'); var a = M.rateLegendArrowHtml(rbaVal, p, 'mortgage', t.good, t.bad); items.push('<span style="display:inline-flex;align-items:center;gap:4px;white-space:nowrap;margin-top:2px;padding-top:2px;border-top:1px solid rgba(148,163,184,0.15);"><span style="display:inline-block;width:14px;height:2px;background:' + t.rba + ';flex-shrink:0;border-radius:1px;"></span><span style="' + M.legendTextStyle('color:' + t.rba + ';opacity:0.8;') + '">RBA</span><span style="' + M.legendTextStyle('color:' + t.rba + ';font-variant-numeric:tabular-nums;font-weight:600;') + '">' + rbaVal.toFixed(2) + '%' + a + '</span></span>'); }
            if (cpiVal != null) { var pc = M.prevStepValue(cpiPoints, ymd || ctxMax, 'value'); var ac = M.rateLegendArrowHtml(Number(cpiVal), pc, 'mortgage', t.good, t.bad, 1); items.push('<span style="display:inline-flex;align-items:center;gap:4px;white-space:nowrap;"><span style="display:inline-block;width:14px;height:0;border-top:2px dashed ' + t.cpi + ';flex-shrink:0;"></span><span style="' + M.legendTextStyle('color:' + t.cpi + ';opacity:0.8;') + '">CPI</span><span style="' + M.legendTextStyle('color:' + t.cpi + ';font-variant-numeric:tabular-nums;font-weight:600;') + '">' + Number(cpiVal).toFixed(1) + '%' + ac + '</span></span>'); }
            return items;
        }
        ...
        // Resize (guard: observer can fire while chart is tearing down; LWC may throw RangeError)
        var disposed = false;
        var resizeObserver = new ResizeObserver(function (entries) {
            ...
            } catch (_e) { /* LWC can stack-overflow if resize races dispose */ }
        });
